	...
	setDesktopwithCameraSource(stream, streamId, onEndedCallback) 
	{
		...
		this.navigatorUserMedia({video: true, audio: false},cameraStream => {
			...
			setInterval(() => {
				...
				canvas.width = screenVideo.videoWidth+cameraWidth+this.camera_margin*2;
				canvas.height = screenVideo.videoHeight;
				canvasContext.drawImage(screenVideo, cameraWidth+this.camera_margin*2, 0, canvas.width, canvas.height);
...
//		this.getDevices();
	 ...
	 */
	 switchVideoCameraCapture(streamId, deviceId, onEndedCallback) 
	 {
		 ...
		 if (videoTrack) {
			...
			alert("There is no video track in local stream")
		 }
	...
	enableAudioLevelForLocalStream(levelCallback, period) {
		...
		soundMeter.connectToSource(this.localStream, function(e) {
			...
			console.log("Added sound meter for stream: " + "xxx" + " = " + soundMeter.instant.toFixed(2));
		});
